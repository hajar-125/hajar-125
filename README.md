# Hajar_Benbassou

# 👋 Hi, I'm Hajar Benbassou

### AI, Data Science & Digital Health Student @ ENSAM Rabat
🎯 Interested in AI Governance, Deep Learning & Computer Vision

<p align="left">
  <img src="https://komarev.com/ghpvc/?username=YOUR_USERNAME&label=Profile%20views&color=0e75b6&style=flat" alt="Views" />
</p>

---

## 🚀 About Me
- 🎓 **Currently:** Pursuing my degree in AI and Digital Health @ ENSAM Rabat.
- 🛡️ **Cybersecurity Focus:** Passionate about **AI for Security (AI4Sec)**, specifically phishing detection and network security.
- 💻 **Focus:** Deep Learning, NLP, and **Secure Socket Programming**.
- 🛠️ **Latest Project:** Joint visual-textual representation using Flickr 8k.
- 🏃 **Fun Fact:** I'm a 10km runner!


###🛡️ Core Tech Stack & Security Tools

<p align="left">
  <img src="https://img.shields.io/badge/Python-FFA586?style=for-the-badge&logo=python&logoColor=161E2F" />
  <img src="https://img.shields.io/badge/PyTorch-B51A2B?style=for-the-badge&logo=pytorch&logoColor=white" />
  <img src="https://img.shields.io/badge/OpenCV-242F49?style=for-the-badge&logo=opencv&logoColor=white" />
  <img src="https://img.shields.io/badge/Cybersecurity-FF0000?style=for-the-badge&logo=dependabot&logoColor=white" />
  <img src="https://img.shields.io/badge/MySQL-FFA586?style=for-the-badge&logo=mysql&logoColor=161E2F" />
  <img src="https://img.shields.io/badge/Linux-B51A2B?style=for-the-badge&logo=linux&logoColor=white" />
</p>

### 🛡️ Featured Security & AI Projects
- **PhishGuard:** A machine learning-driven application designed to detect and block phishing URLs in real-time.
- **Secure FTP Server:** Implemented a custom FTP server using **C and Socket Programming**, focusing on robust network communication.


## 🔥 Featured Projects



## 🌐 Connect With Me

📧 [benbassou.hajar@gmail.com](mailto:your@email.com)
🔗 LinkedIn: [My-Linkedin](https://www.linkedin.com/in/hajar-benbassou-7427202a6/)

---

✨ “Building AI solutions that make impact”
